...
{
	int i, j, k;
	int Nx = 10, Ny = 10, Nz = 10;  // size of the cubic
	double a = 3.2094, c = 5.2108, rc = 6.5;  // size parameters of unit cell
	int x0 = 0, y0 = 0, z0 = 0;  // cordinates of the cubic center
	int Nxc, Nyc, Nzc;  // set the grid [Nxc, Nyc, Nzc] as the center
	
	arma::cube xla, yla, zla;  // cordinates of each grid
	xla.set_size(Ny, Nx, Nz);
	yla.set_size(Ny, Nx, Nz);
	zla.set_size(Ny, Nx, Nz);
	
	Nxc = Nx/2; Nyc=Ny/2; Nzc=Nz/2;  // center grid
